...
Vista para la organización de los siguientes.
"""
from tkinter import *
import tkinter as tk

class ViewSiguientes(object):
    ...
    def textAndInput(self):
        label_1 = Label(self.view,text="SIGUIENTES").place(x=210, y=10)
        self.textBox = Text(self.view, height=15, width=57)
        self.textBox.place(x=20, y=35)

     # Realizar la carga del archivo
    def accionarSig(self):
        info = self.gramatica
        
        for i in info:
           self.textBox.insert("insert","sig(" + i + ")" + " = " + str(info.get(i)) + '\n')
